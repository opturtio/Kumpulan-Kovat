...
class TestBibtexFromatter(unittest.TestCase):
    def setUp(self):
        ...
        self.citation_service.get_citation.return_value = {
            "test_a": 1, "test_b": 2, "test_c": 3, "test_d": 4}
        ...
        self.bibtex_service = BibtexService(
            self.citation_service, self.bibtex_formatter_function)
    ...
    def test_citations_to_bibtex_returns_a_string_containing_the_provided_citations(self):
        return_val = self.bibtex_service._convert_citations_to_bibtex(
            self.citations)
